...
from models.osnet import build_osnet
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head = build_osnet(name="osnet", pretrained=True)
        ...
        faster_rcnn_predictor = FastRCNNPredictor(512, 2)#2048
        reid_head = deepcopy(box_head)
        box_roi_pool = MultiScaleRoIAlign(
            featmap_names=["feat_os4"], output_size=14, sampling_ratio=2
        )
        box_predictor = BBoxRegressor(512, num_classes=2, bn_neck=cfg.MODEL.ROI_HEAD.BN_NECK)#2048
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
       ...
       # print("proposal_features ",proposal_features.shape)
        ...
        #print("proposal_features ",proposal_features["feat_os5"].shape)
        proposal_cls_scores, proposal_regs = self.faster_rcnn_predictor(proposal_features["feat_os5"])
        ...
        box_regs = self.box_predictor(box_features["feat_os5"])
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_os4", "feat_os5"], in_channels=[384, 512], dim=256):
