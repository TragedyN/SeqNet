...
class OSNet(nn.Module):
    ...
    """
    def __init__(self, num_classes=1000, blocks= [OSBlock, OSBlock, OSBlock], layers= [2, 2, 2], channels=[64, 256, 384, 512], feature_dim=512, loss='softmax', IN=False, **kwargs):
    ...
    def featuremaps(self, x):
        out = self.conv1(x)
        print("osnet v1", x.shape)
        out = self.maxpool(out)
        print("osnet maxpool", out.shape)
        out = self.conv2(out)
        print("osnet v2", out.shape)
        out = self.conv3(out)
        print("osnet v3", out.shape)
        out = self.conv4(out)
        print("osnet v4", x.shape)
        out = self.conv5(out)
        print("osnet v5", out.shape)
        # if out.shape[2] == 18:
        #     out = self.gap25(out)
        # elif out.shape[2] == 8:
        #     out = self.gap5(out)
...
def init_pretrained_weights(model, key=''):
    ...
    model_dir = os.path.join(torch_home, 'hub')
...
def osnet_x1_0(num_classes=2, pretrained=True, loss='softmax', **kwargs):
...
class Backbone(nn.Sequential):
        ...
        super(Backbone, self).__init__(
            OrderedDict(
                [
                    ...
                    # ["bn1", osnet.bn1],
                    # ["relu", osnet.relu],
                    ["maxpool", osnet.maxpool],
                    ["layer1", osnet.conv2],  # osnet2
                    ["layer2", osnet.conv3],  # osnet3
                    # ["layer3", osnet.conv4],  # osnet4
                ]
            )
        )
    ...
    def forward(self, x):
        ...
        return OrderedDict([["feat_os4", feat]])

class OS5Head(nn.Sequential):
    def __init__(self, osnet):
        super(OS5Head, self).__init__(OrderedDict([
            # ["layer2", osnet.conv3],  # osnet3
            ["layer3", osnet.conv4],  # osnet4
            ["layer4", osnet.conv5] # osnet5
        ]))
        self.out_channels = [384,512]

    def forward(self, x):
        feat = super(OS5Head, self).forward(x)
        x = F.adaptive_max_pool2d(x, 1)
        feat = F.adaptive_max_pool2d(feat, 1)
        #print("OS5Head feat", feat.shape)
        return OrderedDict([["feat_os4", x], ["feat_os5", feat]])

def build_osnet(name="osnet", pretrained=True):
    osnet = osnet_x1_0()
    #print('osnet', osnet)

    # freeze layers
    osnet.conv1.conv.weight.requires_grad_(False)
    # osnet.bn1.weight.requires_grad_(False)
    # osnet.bn1.bias.requires_grad_(False)

    return Backbone(osnet), OS5Head(osnet)
...
if __name__ == "__main__":
    model = osnet_x1_0()
    image = cv2.imread("/data/lh/datasets/got10k/crop511/test/GOT-10k_Test_000180/00000080.jpg")
    img = cv2.resize(image, (303, 303))

    transform1 = transforms.Compose([transforms.ToTensor()])
    t_img = transform1(img).unsqueeze(0)
    feature_z = model(t_img)
    print("feature_z.shape", feature_z.shape)